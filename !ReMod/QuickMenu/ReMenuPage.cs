...
namespace MLConsoleViewer.QuickMenu
{
    public class ReMenuPage : UiElement, IButtonPage
    {
        ...
        public ReMenuPage AddClickMenu(string name, Action onClick, string tooltip = "", Sprite sprite = null)
        {
            var existingPage = GetMenuPage(name);
            if (existingPage != null)
            {
                return existingPage;
            }
            var clickpage = new ReMenuPage(name);
            AddButton(name, string.IsNullOrEmpty(tooltip) ? $"Open the {name} menu" : tooltip, () =>
            {
                onClick();
                clickpage.Open();
            }, sprite);
